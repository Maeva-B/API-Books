...
async def delete_all_loan(
    ...
    """
    Deletes all existing loan selecting by loan date, return date, book id and/or adherent id.
    Example URL:
      DELETE
        http://localhost/loans?loanDate=2024-10-06&returnDate=2024-12-30&book_id=0&adherent_id=10
    
    Returns the deleted loans number.
    """
    query = {}
    if loanDate :
        if re.match("^[0-9]{4}-[0-1][0-9]-[0-9]{2}$", "{loanDate}" ):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid loan date format")
        query["loanDate"] = loanDate

    if returnDate :
        if re.match("^[0-9]{4}-[0-1][0-9]-[0-9]{2}$", "{returnDate}" ):
            raise HTTPException(
